...
import 'data_cemilan.dart';
import 'detail_resep_page.dart';

class ResepCemilanPage extends StatefulWidget {
  const ResepCemilanPage({super.key});

  @override
  State<ResepCemilanPage> createState() => _ResepCemilanPageState();
}

class _ResepCemilanPageState extends State<ResepCemilanPage> {
  ...
  Future<void> _loadResep() async {
    final list = await ResepDataCemilan.getResepList();
    setState(() {
  ...
  Future<void> _saveResep() async {
    await ResepDataCemilan.saveResepList(resepList);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Resep Cemilan'),
      ),
      body: resepList.isEmpty
          ? const Center(
              child: Text(
                'Belum ada resep cemilan.',
