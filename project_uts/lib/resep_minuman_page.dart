...
import 'data_minuman.dart';
import 'resep_kue_page.dart';
import 'detail_resep_page.dart';

class ResepMinumanPage extends StatefulWidget {
  const ResepMinumanPage({super.key});

  @override
  State<ResepMinumanPage> createState() => _ResepMinumanPageState();
}
...
class _ResepMinumanPageState extends State<ResepMinumanPage> {
  ...
  Future<void> _loadResep() async {
    final list = await ResepDataMinuman.getResepList();
    setState(() {
      resepList = list.isEmpty
          ? []
          : list;
    });
  }
  ...
  Future<void> _saveResep() async {
    await ResepDataMinuman.saveResepList(resepList);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Resep Minuman'),
      ),
